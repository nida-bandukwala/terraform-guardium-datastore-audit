...
# AWS S3 Universal Connector Module Variables
...
variable "force_destroy_bucket" {
  description = "Whether to force destroy the S3 bucket (allows deletion of bucket with objects)"
  type        = bool
  default     = false
}
...
variable "s3_bucket_arns" {
  description = "List of S3 bucket ARNs to monitor for data events. Use [\"arn:aws:s3\"] to monitor all S3 buckets in the account (default), or specify individual buckets like [\"arn:aws:s3:::bucket-name/*\"]"
  type        = list(string)
  default     = ["arn:aws:s3"]
}
